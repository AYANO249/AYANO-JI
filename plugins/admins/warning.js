const handler = async (m, { conn }) => {
  ...
  if (!targetJid || !targetLid) return m.reply('⚠️ *يرجى عمل منشن  إلى المستخدم أو الرد على رسالته* ☺️');

  const user = (await conn.groupMetadata(m.chat)).participants.find(
    p => p.id === targetLid || p.phoneNumber === targetJid
  );

  if (!user) return m.reply("❌ المستخدم غير موجود في هذه المجموعة");
  ...
  await conn.sendMessage(m.chat, {
    text: `⚠️ تم إصدار إنذار جديد

⚜️ المستخدم: @${id.split("@")[0]}
...
handler.before = async (m, { conn }) => {
  ...
  if (g.warnings[user] >= 3) {
    await conn.sendMessage(m.chat, {
      text: `🚫 عذراً @${user.split("@")[0]}، لقد تجاوزت الحد المسموح به من الإنذارات، سيتم طردك الآن لتكرار المخالفة.`,
      mentions: [user]
    }, { quoted: global.reply_status });
