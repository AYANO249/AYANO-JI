handler.before = async (m, { conn }) => {
    ...
    if (m.text.trim().toLowerCase() !== game.answer.toLowerCase()) return;

    clearTimeout(game.timeout);
    delete global.break.games[m.chat];

    if (!global.break.scores[m.chat][player]) {
        global.break.scores[m.chat][player] = 0;
    }
    ...
    await m.reply(`✅ أحسنت! معاك: ${global.break.scores[m.chat][player]} نقطة`);
...
async function handler(m, { conn }) {
    ...
    try {
        data = await (await fetch("https://raw.githubusercontent.com/Xov445447533/Xov11111/master/src/JSON/venom-تفكيك.json")).json();
    } catch (e) {
        return m.reply("❌ خطأ في تحميل الأسئلة");
    }
    ...
    const img = "https://i.ibb.co/ymYdcdNF/f14281b39364343a3ac05c8ed58f0c29.jpg";
...
> _اكتب الإجابة بسرعة للحصول على نقطة_
> _الوقت: 30 ثانية_
`
    });
